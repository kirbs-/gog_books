...
class Book(db.Model):
    ...
    @staticmethod
    def highest_rated():
        return Book.query.filter(Book.ignore == None).join(Rating).filter_by(source='amazon' ).order_by(Rating.value.desc(), Book.id).all()

    @staticmethod
    def most_reviewed():
        return Book.query.filter(Book.ignore == None).join(Rating).filter_by(source='amazon').order_by(Rating.review_count.desc(), Book.id).all()

    @staticmethod
    def newest():
        return Book.query.filter(Book.ignore == None ).join(Show).order_by(Show.date.desc(), Book.id).all()
